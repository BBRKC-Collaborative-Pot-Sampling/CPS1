...
# PROCESS DATA ----------------------------------------------------------------------------------------------------------------
  ...
  # Expand specimen biometric table, join to raw_specimen table to get catch sample ID, join with samples file to add
  # number tossed
      raw_specimen_bio %>%
  ...
  # file to get lat/lon, set/haul date and time for each pot (with positive catch)
   ...
   # Update catch summary table with new crab #s from sampling factor
    specimen_table %>%
        group_by(CRUISE, VESSEL, POT, SPECIES_CODE) %>%
        summarise(NUMBER_CRAB = sum(SAMPLING_FACTOR)) %>%
        right_join(catch %>% rename(POT = HAUL, N_ENTRIES = NUMBER_CRAB)) %>%
...
# ERROR CHECKING -----------------------------------------------------------------------------------------------------
  ...
  # Write function
  error_chk <- function(specimen_table, catch_summary){
    ...
    print("CHECKING DISEASE CODES...")
      ...
      unique(specimen_table$DISEASE_CODE == 1 & (is.na(specimen_table$DISEASE_DORSAL) == TRUE &
                                                   is.na(specimen_table$DISEASE_LEGS) == TRUE &
                                                   is.na(specimen_table$DISEASE_VENTRAL) == TRUE)) -> A16
      print(A16)
      
      print("17) Any bitter crab recorded for RKC and/or any bitter crab recorded with entries in % coverage?")
      unique(specimen_table$DISEASE_CODE == 2 |
               specimen_table$DISEASE_CODE == 2 & (is.na(specimen_table$DISEASE_DORSAL) == FALSE &
                                                     is.na(specimen_table$DISEASE_LEGS) == FALSE &
                                                     is.na(specimen_table$DISEASE_VENTRAL) == FALSE)) -> A17
      print(A17)
      
      print("18) Any disease code not recorded but entries in % coverage?")
      unique(is.na(specimen_table$DISEASE_CODE) == TRUE & (is.na(specimen_table$DISEASE_DORSAL) == FALSE &
                                                             is.na(specimen_table$DISEASE_LEGS) == FALSE &
                                                             is.na(specimen_table$DISEASE_VENTRAL) == FALSE)) -> A18
      print(A18)
      
      print("19) Any disease codes >9?")
      unique(specimen_table$DISEASE_CODE>9) -> A19
      print(A19)
    
    print("CHECKING SAMPLING FACTOR")
    
      print("20) What is the maximum sampling factor by by sex?")
      specimen_table %>%
        group_by(SPECIES_CODE, SEX) %>%
        summarise(MIN_SAMPLING_FACTOR = min(SAMPLING_FACTOR),
                  MAX_SAMPLING_FACTOR= max(SAMPLING_FACTOR)) %>%
        as.data.frame() -> A20
      print(A20)
      
      print("21) Any sampling factors < 1?")
      unique(specimen_table$SAMPLING_FACTOR < 1) -> A21
      print(A21)
    
    # Check station ids (bouys)
    ...
    print("22) Does the number of crab and number of entries match between the specimen table and catch summary?")
    all.equal(spec_sum, catch_sum) -> A22
    print(A22)
  }    
  
  # Run function
  error_chk(specimen_table, catch_summary) 
