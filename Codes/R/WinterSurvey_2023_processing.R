...
# Map cpue for Bristol Bay -----------------
  ...
  pot_cpue %>%
    ...
    mutate(MAT_SEX = factor(MAT_SEX, levels = c("Legal male", "Sublegal male", "Mature female", "Immature female")),
           nonzero = ifelse(CPUE == 0, "N", "Y")) %>%
    sf::st_as_sf(coords = c(x = "LONGITUDE", y = "LATITUDE"), 
                 crs = sf::st_crs(4326)) %>% 
    sf::st_transform(crs = coldpool:::ebs_proj_crs) %>%
    #mutate(CPUE = ifelse(CPUE == 0, NA, CPUE)) %>%
    ggplot() +
    #geom_sf(data = map_layers$bathymetry, color=alpha("grey70")) +
    geom_sf(aes(size=CPUE, shape = nonzero), stat="identity", position="identity")+
    scale_shape_manual(values = c(10, 19))+
    scale_size(range = c(2, 10))+
    geom_sf(data = st_as_sf(BB_strata), fill = NA, color = "black") +
    ...
    guides(size=guide_legend(title="CPUE"), shape = "none") +
    ...
    geom_text(data = akgfmaps::transform_data_frame_crs(
      data.frame(label = paste("Last pull date:",max.date),
                 MAT_SEX = as.factor(c("Legal male", "Sublegal male", "Mature female", "Immature female")),
                 x = c(-163.5, 0, 0, 0),
                 y = c(58.75, 0, 0, 0)), 
                 out.crs = coldpool:::ebs_proj_crs), 
      mapping = aes(x = x, y = y, label = label, fontface = "bold"), size = 3.5, color = c("blue", NA, NA, NA))+
    theme_bw() +
